...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    /// </summary>
    public class XRKnobBaShou : XRBaseInteractable
    {
        ...
        //自加
        [SerializeField]
        private bool isTrueDoor = false;

        [SerializeField] 
        private AudioClip m_Aside;
        [SerializeField]
        private float baShouChangeTime;
        [SerializeField]
        ...
        IEnumerator BaShouReload(bool openComplete = false)
        {
            while (m_Handle.rotation.eulerAngles.z <= 179.999f)
            {
                Quaternion angle = Quaternion.AngleAxis(baShouChangeTime, new Vector3(0,0,1));
                m_Handle.rotation *= angle;
                yield return null;
            }
            m_Handle.GetComponent<BoxCollider>().enabled = true;
            m_Value = 1;
            if(openComplete)
            {
                this.enabled = false;
        ...
        IEnumerator BaShouReloadStartDoor()
        {
            while (m_Handle.rotation.eulerAngles.z >= 0.01f)
            {
                Quaternion angle = Quaternion.AngleAxis(baShouChangeTime, new Vector3(0,0,-1));
                m_Handle.rotation *= angle;
                yield return null;
        ...
        void EndGrab(SelectExitEventArgs args)
        {
            if (isTrueDoor)
            {
                if (m_Value <= 0.01f)
                {
                    m_Handle.GetComponent<BoxCollider>().enabled = false;
                    //开门特殊事件
                    StartCoroutine(BaShouReload(true));
                }
                else
                {
                    m_Handle.GetComponent<BoxCollider>().enabled = false;
                    StartCoroutine(BaShouReload());
                }
            }
            else if (!isTrueDoor)
            {
                ...
                {
                    m_Handle.GetComponent<BoxCollider>().enabled = false;
                    this.GetComponent<AudioSource>().PlayOneShot(m_Aside);
                    StartCoroutine(BaShouReloadStartDoor());      
                }
                else
                {
                    StartCoroutine(BaShouReloadStartDoor());
        ...
        void SetKnobRotation(float angle)
        {
            ...
            if (m_Handle != null)
                m_Handle.localEulerAngles = new Vector3(-180.0f, -90.0f, angle);
            if (m_Value <= 0.01f && isTrueDoor)
            {
